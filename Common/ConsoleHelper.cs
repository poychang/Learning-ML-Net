...
namespace Common
{
    public static class ConsoleHelper
    {
        ...
        {
            var msg =
                $"Peek data in DataView: Showing {numberOfRows.ToString()} rows with the columns specified by TObservation class";
        ...
        public static List<float[]> PeekVectorColumnDataInConsole(MLContext mlContext, string columnName, IDataView dataView, IEstimator<ITransformer> pipeline, int numberOfRows = 4)
        {
            ...
            var someColumnData = transformedData
                .GetColumn<float[]>(mlContext, columnName)
                .Take(numberOfRows)
                .ToList();
